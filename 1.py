...
# A função pega o vendedor e verifica a comissão e já soma
def organizar_por_comissoes(vendas):
    for i in vendas:

        if i["valor"] < 100:
            i["comissão"] = 0
        elif 500 <= i["valor"] <= 1000:
            i["comissão"] = i["valor"] * 0.01
        else:
            i["comissão"] = i["valor"] * 0.05
    return vendas

def somar_comissoes(vendas):
    comissoes_totais = {}
    for venda in vendas:
        vendedor = venda["vendedor"]
        comissao = venda.get("comissão", 0)
        if vendedor in comissoes_totais:
            comissoes_totais[vendedor] += comissao
        else:
            comissoes_totais[vendedor] = comissao
    return comissoes_totais 
print(somar_comissoes(organizar_por_comissoes(vendas)))